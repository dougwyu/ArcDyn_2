...
# testseqsall <- read.fasta(file = "GMGLM411_13.fas", seqtype = "DNA", set.attributes = FALSE, as.string = TRUE, forceDNAtolower = FALSE)  # read in fasta file using seqinr
...
# #### bold_identify_parents one sequence at a time (for large files that take a long time and might crash partway, this saves the output of each sequence)
# testseqsallbold_parents <- vector('list')
# for (i in 1:length(testseqsallbold)) {
#     print(i)
#     boldoutput <- bold_identify_parents(testseqsallbold[i], db = "COX1")
...
write_tsv(testseqsallbold_parents_96_to_100_413seqs.df, "CO1_1sequence_perBIN_040915_COIspiking_96_to_100.tsv")
