...
# Usage: bash _loop_samtools_only_20180417.sh

# Interactive procedure:  first run these commands if running _loop_samtools_only_20180417.sh interactively
...
# Batch procedure:  bsub script and submit to mellanox-ib queue
	######### _loop_samtools_only_20180417.bsub ##############################################################################
	#######################################################################################
     #!/bin/sh
     ...
     # PATH=$PATH:~/scripts/bedtools2/bin # made 22 Jan 2018 from github 2.27.1
     #
     # bash _loop_samtools_only_20180219.sh
     #######################################################################################
     #######################################################################################
...
# SCRIPT OUTLINE:
  ...
  # move the result to bwa_outputs
	# rm working folder
  # next folder

PIPESTART=$(date)

# upload _loop_samtools_only_20180417.sh to BWA## folder that contains the bam files that i want to filter
# I don't place the shell script in the bwa_outputs because it's harder to manage the files
HOMEFOLDER=$(pwd) # this sets working directory to that folder.

echo "Home folder is ${HOMEFOLDER}"
cd "${HOMEFOLDER}"/bwa_outputs/ || exit  # cd into bwa_outputs folder
...
sample_info=folderlist.txt # put folderlist.txt into variable. folderlist.txt should already be in the bwa_outputs folder
