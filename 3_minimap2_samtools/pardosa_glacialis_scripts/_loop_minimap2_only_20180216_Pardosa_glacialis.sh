...
# SCRIPT OUTLINE:
     ...
     # move the outputs to pardosa_glacialis_outputs
     # rm working folder
     # next folder
...
# set variables
INDEX=1
if [ ! -d pardosa_glacialis_outputs ] # if directory pardosa_glacialis_outputs does not exist.  this is within the HOMEFOLDER
then
	mkdir pardosa_glacialis_outputs
fi
...
do
     ...
     # against Pardosa glacialis only
     minimap2 -ax sr ~/greenland_2017/reference_seqs/Pardosa_glacialis.fas ${sample}_R1_val_1.fq.gz ${sample}_R2_val_2.fq.gz | samtools view -b | samtools sort -@27 - -o ${sample}_Pardosa_glacialis_sorted.bam
     ...
     samtools index ${sample}_Pardosa_glacialis_sorted.bam # creates ${sample}_sorted.bam.bai file
     samtools flagstat ${sample}_Pardosa_glacialis_sorted.bam > ${sample}_Pardosa_glacialis_sorted.bam.flagstat.txt # basic stats

     echo "**** moving outputs to pardosa_glacialis_outputs/"
     mv ${sample}_Pardosa_glacialis_sorted.bam.bai ../pardosa_glacialis_outputs/
     mv ${sample}_Pardosa_glacialis_sorted.bam.flagstat.txt ../pardosa_glacialis_outputs/
     mv ${sample}_Pardosa_glacialis_sorted.bam ../pardosa_glacialis_outputs/
...
mv folderlist.txt pardosa_glacialis_outputs/
