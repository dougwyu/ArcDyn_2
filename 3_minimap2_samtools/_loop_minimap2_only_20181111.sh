...
# Usage: bash _loop_minimap2_only_20181111.sh
# this version uses the 308 mitogenome reference fasta plus 41 COI+CytB seqs
...
# Batch procedure:  bsub script and submit to mellanox-ib queue
     ######### _loop_minimap2_only_20180528.bsub ##############################################################################
     #######################################################################################
     # #!/bin/sh
...
# upload _loop_minimap2_only_20181111.sh *into* folder that contains the sample folders that i want to map
...
do
     ...
     minimap2 -t 16 -ax sr ~/greenland_2017/reference_seqs/ArcDyn_308_full_mitogenomes_and_41_COI_CYTB_20181109.fasta ${sample}_R1_val_1.fq.gz ${sample}_R2_val_2.fq.gz | samtools view -b | samtools sort -@15 - -o ${sample}_sorted.bam
