...
# Usage: bash _loop_samtools_only_Pardosaspp_20180317.sh
...
# Batch procedure:  bsub script and submit to mellanox-ib queue
	...
	#######################################################################################
     ...
     #
     # bash _loop_samtools_only_Pardosaspp_20180317.sh
     #######################################################################################
     #######################################################################################
...
# SCRIPT OUTLINE:
  ...
  # move the result to pardosa_spp_outputs
	# rm working folder
  # next folder

PIPESTART=$(date)

# upload _loop_samtools_only_Pardosaspp_20180317.sh to BWA## folder that contains the bam files that i want to filter
# I don't place the shell script in the pardosa_spp_outputs because it's harder to manage the files
HOMEFOLDER=$(pwd) # this sets working directory to that folder.

echo "Home folder is ${HOMEFOLDER}"
cd "${HOMEFOLDER}"/pardosa_spp_outputs/ || exit  # cd into pardosa_spp_outputs folder
...
sample_info=folderlist.txt # put folderlist.txt into variable. folderlist.txt should already be in the pardosa_spp_outputs folder
