...
# Usage: bash _loop_samtools_only_20180219.sh
...
# SCRIPT OUTLINE:
  ...
  # move the result to minimap2_outputs_19posctrls
	# rm working folder
  # next folder

PIPESTART=$(date)

# upload _loop_samtools_only_20180219.sh to BWA## folder that contains the bam files that i want to filter
# I don't place the shell script in the minimap2_outputs_19posctrls because it's harder to manage the files
HOMEFOLDER=$(pwd) # this sets working directory to that folder.

echo "Home folder is ${HOMEFOLDER}"
cd "${HOMEFOLDER}"/minimap2_outputs_19posctrls/ || exit  # cd into minimap2_outputs_19posctrls folder
...
sample_info=folderlist.txt # put folderlist.txt into variable. folderlist.txt should already be in the minimap2_outputs_19posctrls folder
