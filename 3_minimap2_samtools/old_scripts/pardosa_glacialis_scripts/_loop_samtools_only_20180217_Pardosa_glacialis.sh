...
# SCRIPT OUTLINE:
  ...
  # move the result to pardosa_glacialis_outputs
	# rm working folder
  # next folder

PIPESTART=$(date)

# upload _loop_samtools_only_20180217_Pardosa_glacialis.sh to BWA## folder that contains the bam files that i want to filter
# I don't place the shell script in the pardosa_glacialis_outputs because it's harder to manage the files
HOMEFOLDER=$(pwd) # this sets working directory to that folder.

echo "Home folder is ${HOMEFOLDER}"
cd "${HOMEFOLDER}"/pardosa_glacialis_outputs/ || exit  # cd into pardosa_glacialis_outputs folder
...
sample_info=folderlist.txt # put folderlist.txt into variable. folderlist.txt should already be in the pardosa_glacialis_outputs folder
...
do
     ...
     samtools view -b -F 2308 -q 1 ${sample}_Pardosa_glacialis_sorted.bam -o ${sample}_F2308_q1_sorted.bam
     ...
     #### samtools filtering pipeline using -F 2308 -q 60 ####
          ...
          # see https://broadinstitute.github.io/picard/explain-flags.html;  see also https://www.biostars.org/p/256448/
          # samtools view -q 1 # to remove multiply mapped reads = q 0
     samtools view -b -F 2308 -q 60 ${sample}_Pardosa_glacialis_sorted.bam -o ${sample}_F2308_q60_sorted.bam
