...
# SCRIPT OUTLINE:
     ...
     # move the outputs to pardosa_spp_outputs
     # rm working folder
     # next folder
...
# set variables
INDEX=1
if [ ! -d pardosa_spp_outputs ] # if directory pardosa_spp_outputs does not exist.  this is within the HOMEFOLDER
then
...
do
     ...
     # against Pardosa spp only
     minimap2 -ax sr ~/greenland_2017/reference_seqs/Pardosa_spp_derep.fas ${sample}_R1_val_1.fq.gz ${sample}_R2_val_2.fq.gz | samtools view -b | samtools sort -@27 - -o ${sample}_Pardosa_spp_sorted.bam
     ...
     samtools index ${sample}_Pardosa_spp_sorted.bam # creates ${sample}_sorted.bam.bai file
     samtools flagstat ${sample}_Pardosa_spp_sorted.bam > ${sample}_Pardosa_spp_sorted.bam.flagstat.txt # basic stats

     echo "**** moving outputs to pardosa_spp_outputs/"
     mv ${sample}_Pardosa_spp_sorted.bam.bai ../pardosa_spp_outputs/
     mv ${sample}_Pardosa_spp_sorted.bam.flagstat.txt ../pardosa_spp_outputs/
     mv ${sample}_Pardosa_spp_sorted.bam ../pardosa_spp_outputs/
