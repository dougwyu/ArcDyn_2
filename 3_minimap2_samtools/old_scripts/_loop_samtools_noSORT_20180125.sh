...
# Usage: bash _loop_samtools_only_20180123.sh
...
# Batch procedure:  bsub script and submit to mellanox-ib queue
	######### loop_samtools_only_20180123.bsub ##############################################################################
	#######################################################################################
     #!/bin/sh
     ...
     # PATH=$PATH:~/scripts/bedtools2/bin # made 22 Jan 2018 from github 2.27.1
     #
     # bash _loop_samtools_only_20180123.sh
     #######################################################################################
     #######################################################################################
...
# upload _loop_samtools_only_20180123.sh to BWA## folder that contains the bam files that i want to filter
...
do
     ...
     #### NO NEED FOR THIS BECAUSE ASSUMING ALREADY DONE IN _loop_samtools_only_20180123.sh
     ...
     #### samtools filtering pipeline using -F 2308 -f 0x2 -q 1 ####
     ...
     samtools view -b -F 2308 -f 0x2 -q 1 ${sample}_sorted.bam -o ${sample}_F2308_f0x2_q1_sorted.bam
     samtools index ${sample}_F2308_f0x2_q1_sorted.bam # creates ${sample}_F2308_f0x2_q1_sorted.bam.bai file
     samtools idxstats ${sample}_F2308_f0x2_q1_sorted.bam > ${sample}_F2308_f0x2_q1_sorted.bam_idxstats.txt # calculate idxstats
     bedtools genomecov -d -ibam ${sample}_F2308_f0x2_q1_sorted.bam | gzip > ${sample}_F2308_f0x2_q1_sorted_genomecov_d.txt.gz # calc genome cov stats

     #### samtools filtering pipeline using -F 2308 -f 0x2 -q 60 ####
          # samtools view -f 0x2 # keep PROPER_PAIR   .. each segment properly aligned according to the aligner
          # samtools view -F 2308 # exclude UNMAP,SECONDARY,SUPPLEMENTARY # exclude unmapped, secondary, and supplementary alignments
          # see https://broadinstitute.github.io/picard/explain-flags.html;  see also https://www.biostars.org/p/256448/
          # samtools view -q 1 # to remove multiply mapped reads = q 0
     samtools view -b -F 2308 -f 0x2 -q 60 ${sample}_sorted.bam -o ${sample}_F2308_f0x2_q60_sorted.bam
     samtools index ${sample}_F2308_f0x2_q60_sorted.bam # creates ${sample}_F2308_f0x2_q60_sorted.bam.bai file
     samtools idxstats ${sample}_F2308_f0x2_q60_sorted.bam > ${sample}_F2308_f0x2_q60_sorted.bam_idxstats.txt # calculate idxstats
     bedtools genomecov -d -ibam ${sample}_F2308_f0x2_q60_sorted.bam | gzip > ${sample}_F2308_f0x2_q60_sorted_genomecov_d.txt.gz # calc genome cov stats
