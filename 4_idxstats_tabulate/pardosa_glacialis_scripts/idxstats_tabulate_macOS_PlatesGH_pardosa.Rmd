...
idxstatsgenomecovfolder <- paste0("minimap2_all_outputs_PlatesGH_", samtoolsfilter)  # this is the enclosing folder around the idxstats and genomcov files for a given set of soups

# use this version for COI_barcodes mapping
# idxstatsgenomecovfolder <- paste0("minimap2_all_outputs_PlatesGH_", samtoolsfilter, "_COIBarcodesOnly")  # this is the enclosing folder around the idxstats and genomcov files for a given set of soups
...
# dim(idx)
# sanity checks
idx %>% distinct(well) %>% dplyr::count()  # 96, number of distinct values of well, as a sanity check
idx %>% distinct(sample) %>% dplyr::count()  # number of distinct values of sample:  190
idx %>% distinct(pathname) %>% dplyr::count()  # number of distinct values of sample:  190
idx %>% distinct(samtools_filter) %>% dplyr::count() # number of distinct values of samtools_filter: should be 1
idx %>% distinct(samtools_filter) # should look like:  F2308_f0x2_q1

idx_mitogenome_table <- idx %>% group_by(well) %>% distinct(mitogenome) %>% dplyr::count() %>% arrange(desc(n)); View(idx_mitogenome_table) # should be 160

length(idx$mitogenome) / idx_mitogenome_table[1,2] == idx %>% distinct(sample) %>% dplyr::count()  # should evaluate to TRUE
# 30400 rows / 160 mitogenomes = 190 samples
```
...
# Convert Year + Week number to Year + Month + Date, where 
# https://community.exploratory.io/t/how-to-convert-week-number-to-date/578
samplemetadata <- samplemetadata %>% dplyr::mutate(YearWeeknum = str_c(Year, str_replace(Week, "Wk", "-")))
samplemetadata <- samplemetadata %>% dplyr::mutate(Date = as.Date(str_c(YearWeeknum, "-1"), format = "%Y-%U-%u"))
samplemetadata <- samplemetadata %>% tidyr::unite("Full_name_of_the_sample", c("Date", "Plot", "Trap", "Week"), remove = FALSE)
samplemetadata <- samplemetadata %>% dplyr::select(Full_name_of_the_sample, Date, Short_name_of_the_sample, Plate:Extraction_kit_protocol_used)
...
samplemetadata %>% distinct(well) %>% dplyr::count()  # 96, number of distinct values of well, as a sanity check
...
genomecoverages_summ <- purrr::map(genomecov_files, loadFile2); 
notify("Your loadfile2 function has finally finished!", "Message from R", speakIt=TRUE)
...
# saveRDS(genomecoverages_summ, "genomecoverages_summ_PlatesGH_20180214.RDS", compress = TRUE)
# genomecoverages_summ <- readRDS("GH_outputs_20180214_F2308_f0x2_q60/genomecoverages_summ_PlatesGH_20180214.RDS")
# 
...
#  [5] "mitogenome"                 "COI_Species"                "mock_ArcDyn"                "mapped_reads"              
#  [9] "mt_length"                  "sample"                     "well"                       "samtools_filter"           
# [13] "pathname"                   "Full_name_of_the_sample"    "Year"                       "Short_name_of_the_sample"  
# [17] "Plate"                      "Sample_alias"               "EI_sample_name"             "Concentration_ng_ul"       
# [21] "Sample_volume_ul"           "qty_loaded_uL"              "Quantification_method"      "x.260_280_ratio"           
# [25] "lysis_buffer_orig_total_ul" "lysis_buffer_purified_ul"   "lysis_buffer_proportion"    "sum_coverage"              
# [29] "mean_coverage"              "stddev"                     "coefvar"                    "length"                    
# [33] "pathname_genomecov"     
```
...
# filter out samples with COI_spike_sum < 100 000 (summed over all 160 mitogenomes in the sample).  This removes 5 samples with very small numbers of COI_spike_sum including the small ME50_1 mock community which seems to have failed

sample_names <- sum_idx_meta_genomecov %>% dplyr::filter(COI_spike_sum < 100000) %>% dplyr::select(Short_name_of_the_sample)
...
idx_meta_genomecov <- idx_meta_genomecov %>% filter(!Short_name_of_the_sample %in% samples_to_remove) # 29045 obs
...
# [43] "elaterid_mordellid_sum"                      "COI_corr"                         
```
...
# write_tsv(mocks_idx_meta_genomecov, "mocks_idx_meta_genomecov_PlatesGH_20180214.txt") # so if i need to read in a file during debuggging, here it is
# mocks_idx_meta_genomecov <- read_delim("mocks_idx_meta_genomecov_PlatesGH_20180214.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE)

# remove mock soup data from idx_meta_genomecov
idx_meta_genomecov <- idx_meta_genomecov %>% dplyr::filter(mock_ArcDyn == "arcdyn") # 27475 rows
...
# commented out because i have month and date information now # create variable from Year, Week, and Trap(ABC) 
# stringr::str_pad(x, width = 2, side = "left", pad = "0")  # to ensure that the date always has two digits, e.g. 07.
...
# write_tsv(idx_meta_genomecov, "idx_meta_genomecov_PlatesGH_20180214.txt") # so if i need to read in a file during debuggging, here it is
# used for debugging, in case i screw up the dataset while writing code
# idx_meta_genomecov <- read_delim("idx_meta_genomecov_PlatesGH_20180214.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
```
...
# write_tsv(idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide, "idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide_PlatesGH_20180214.txt")
...
# write_tsv(idx_meta_genomecov_mapped_reads_COI_lysis_corr_coefvarmin_wide, "idx_meta_genomecov_mapped_reads_COI_lysis_corr_coefvarmin_wide_PlatesGH_20180214.txt")
...
mocks_idx_meta_genomecov_coefvar_wide <- mocks_idx_meta_genomecov %>% dplyr::select(Short_name_of_the_sample, mitogenome, coefvar) %>% dplyr::arrange(Short_name_of_the_sample) %>% tidyr::spread(Short_name_of_the_sample, coefvar) %>% dplyr::arrange(mitogenome)
...
# write_tsv(mocks_idx_meta_genomecov_mapped_reads_COI_corr_wide, "mocks_idx_meta_genomecov_mapped_reads_COI_corr_wide_PlatesGH_20180214.txt")
# 
# write_tsv(mocks_idx_meta_genomecov_coefvar_wide, "mocks_idx_meta_genomecov_coefvar_wide_PlatesGH_20180214.txt")
# 
# write_tsv(mocks_idx_meta_genomecov_mapped_reads_wide, "mocks_idx_meta_genomecov_mapped_reads_wide_PlatesGH_20180214.txt")
```
