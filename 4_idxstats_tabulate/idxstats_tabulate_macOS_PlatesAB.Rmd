...
# outputs_PlatesAB_F2308_f0x2_q48_bwa_20180526/
# idxstatsgenomecovfolder <- paste0("bwa_outputs_PlatesAB_", samtoolsfilter, "_", samtoolsqual, "_bwa_20180526")  # this is the enclosing folder around the idxstats and genomcov files for a given set of soups

# use this version for minimap2
# outputs_PlatesAB_F2308_f0x2_q48_minimap2_20180527/
idxstatsgenomecovfolder <- paste0("outputs_PlatesAB_", samtoolsfilter, "_", samtoolsqual, "_minimap2_20180527")  # this is the enclosing folder around the idxstats and genomcov files for a given set of soups
...
idx_mitogenome_table <- idx %>% group_by(well) %>% distinct(mitogenome) %>% count() %>% arrange(desc(n)); View(idx_mitogenome_table) # should be 311 in all rows

idx_mitogenome_table2 <- idx %>% group_by(well) # %>% distinct(mitogenome) %>% count() %>% arrange(desc(n)); View(idx_mitogenome_table2) # should be 311 in all rows
...
# saveRDS(genomecoverages_summ, "genomecoverages_summ_PlatesAB_minimap2_20180527.RDS", compress = TRUE)
# genomecoverages_summ <- readRDS("genomecoverages_summ_PlatesAB_minimap2_20180527.RDS")
...
# sanity check:  189 samples, which is correct since i removed 1 sample
...
# write_tsv(idx_meta_genomecov, "idx_meta_genomecov_PlatesAB_20180527.txt") # so if i need to read in a file during debuggging, here it is. This is the file for HMSC analysis.
# used for debugging, in case i screw up the dataset while writing code
# idx_meta_genomecov <- read_delim("idx_meta_genomecov_PlatesAB_20180527.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
# if i need to read from a folder
# # idx_meta_genomecov <- read_delim("AB_outputs_20180214_F2308_f0x2_q60/idx_meta_genomecov_PlatesAB_20180527.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
```
...
# write_tsv(idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide, "idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide_PlatesAB_20180527.txt")
...
# uncomment to save table
# write_tsv(idx_meta_genomecov_mapped_reads_COI_lysis_corr_pctcovmin_wide, "idx_meta_genomecov_mapped_reads_COI_lysis_corr_pctcovmin_wide_PlatesAB_20180527.txt")
```
