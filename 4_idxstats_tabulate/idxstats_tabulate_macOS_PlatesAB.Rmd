...
idx_mitogenome_table <- idx %>% group_by(well) %>% distinct(mitogenome) %>% count() # should be 160

# 30720/160 = 192 # 30720 rows / 160 mitogenomes = 192 samples = 2 
...
# genomecov_files <- list.files(Sys.glob(file.path(idxstatsgenomecovfolder, "BWA??", folder2)), pattern = genomecovfile, full.names = TRUE) # previous version
...
# saveRDS(genomecoverages_summ, "genomecoverages_summ_20180209.RDS", compress = TRUE)
# genomecoverages_summ <- readRDS("genomecoverages_summ_20180209.RDS")
...
# [17] "TGAC_barcode"               "Concentration_ng_per_ul"    "Sample_volume_ul"           "Quantification_method"     
# [21] "x.260_280_ratio"            "lysis_buffer_orig_total_ul" "lysis_buffer_purified_ul"   "lysis_buffer_proportion"   
# [25] "sum_coverage"               "mean_coverage"              "stddev"                     "coefvar"                   
# [29] "length"                     "pathname_genomecov"  

# write_tsv(idx_meta_genomecov, "idx_meta_genomecov_PlatesAB.txt") # so if i need to read in a file during debuggging, here it is
...
idx_meta_genomecov <- read_delim("idx_meta_genomecov_PlatesAB.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
...
```{r}
idx_meta_genomecov$mapped_reads <- as.numeric(idx_meta_genomecov$mapped_reads)
...
sum_idx_meta_genomecov <- sum_idx_meta_genomecov %>%
    mutate(elaterid_mordellid_sum=elaterid_mordellid_sum/n_distinct(idx_meta_genomecov$mitogenome))
...
# filter out samples with mapped_reads<2000 to begin with

sample_names <- sum_idx_meta_genomecov %>% dplyr::filter(mapped_reads < 2000) %>% dplyr::select(Full_name_of_the_sample)
...
# write_tsv(idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide, "idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide_PlatesAB.txt")
```
...
11. create lysis buffer datasets, download idxstats and genomecov data, run R code on Plates AB, A2B2, EF, GH:  COMPARE PlatesAB and A2B2 WITH YINQIU'S BWA_results_20161025.xlsx.  Compare PlatesGH with the positive controls, the seasonal patterns, the COI spikes
