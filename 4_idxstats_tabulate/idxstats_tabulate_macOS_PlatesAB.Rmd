...
idxstatsgenomecovfolder <- "minimap2_all_outputs_PlatesAB"  # this is the enclosing folder around the idxstats and genomcov files for a given set of soups
...
# saveRDS(genomecoverages_summ, "genomecoverages_summ_20180210.RDS", compress = TRUE)
# genomecoverages_summ <- readRDS("genomecoverages_summ_20180210.RDS")
...
# filter out samples with elaterid_mordellid_sum < 10612886 to begin with

sample_names <- sum_idx_meta_genomecov %>% dplyr::filter(elaterid_mordellid_sum < 10612886) %>% dplyr::select(Full_name_of_the_sample)
samples_to_remove <- sample_names %>% dplyr::pull(Full_name_of_the_sample) # pull out a single variable
samples_to_remove # 3 samples to remove
idx_meta_genomecov <- idx_meta_genomecov %>% filter(!Full_name_of_the_sample %in% samples_to_remove) # 29673 + 471 = 30144
...
# write_tsv(idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide, "idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide_PlatesAB_20180210.txt")
```
