...
idxstatsgenomecovfolder <- paste0("minimap2_all_outputs_PlatesAB_", samtoolsfilter, "_", samtoolsqual)  # this is the enclosing folder around the idxstats and genomcov files for a given set of soups

# use this version for COI_barcodes mapping
# idxstatsgenomecovfolder <- paste0("minimap2_all_outputs_PlatesAB_", samtoolsfilter, "_COIBarcodesOnly")  # this is the enclosing folder around the idxstats and genomcov files for a given set of soups
...
idx %>% distinct(samtools_filter)  # show samtools filter value:  should be something like F2308_f0x2_q1
...
# later, will *multiply* the mapped_reads column by lysis_buffer_proportion to normalise by total amount of lysis buffer used to digest the sample
```
...
# [17] "TGAC_barcode"               "Concentration_ng_per_ul"    "Sample_volume_ul"           "Quantification_method"     
...
# saveRDS(genomecoverages_summ, "genomecoverages_summ_20180220_F2308_q48.RDS", compress = TRUE)
# genomecoverages_summ <- readRDS("genomecoverages_summ_20180220_F2308_q48.RDS")
...
# [17] "EI_Plate_name"              "TGAC_barcode"               "Concentration_ng_per_ul"    "Sample_volume_ul"          
...
# should be 2, but it's 3.67, sd = 0.72
...
# should be 4, but it's 7.16, sd = 1.50
...
# should be 2, and it's 1.95, sd = 0.07
...
sum_idx_meta_genomecov <- idx_meta_genomecov %>% 
    ...
    summarise_at(vars(mapped_reads, sum_coverage, elaterid_mordellid_sum), funs(sum(as.numeric(.)))) %>%
    arrange(elaterid_mordellid_sum)
sum_idx_meta_genomecov <- sum_idx_meta_genomecov %>%
    mutate(species_reads_per_COIspike=mapped_reads/elaterid_mordellid_sum)
# sum_idx_meta_genomecov <- sum_idx_meta_genomecov %>%
#     mutate(elaterid_mordellid_sum=elaterid_mordellid_sum/n_distinct(idx_meta_genomecov$mitogenome))  # not necessary

plot(mapped_reads ~ elaterid_mordellid_sum, data = sum_idx_meta_genomecov)
...
hist(sum_idx_meta_genomecov$elaterid_mordellid_sum, breaks = 20000, xlim = c(0, 1005599758))
hist(sum_idx_meta_genomecov$elaterid_mordellid_sum, breaks = 20000, xlim = c(0, 100559900))
par(mfrow=c(1,1))

# filter out samples with elaterid_mordellid_sum < 9 000 000 to begin with

sample_names <- sum_idx_meta_genomecov %>% dplyr::filter(elaterid_mordellid_sum < 9000000) %>% dplyr::select(Full_name_of_the_sample)
...
idx_meta_genomecov <- idx_meta_genomecov %>% filter(!Full_name_of_the_sample %in% samples_to_remove) # 29987 + 157 = 30144
...
# write_tsv(idx_meta_genomecov, "idx_meta_genomecov_PlatesAB_20180220.txt") # so if i need to read in a file during debuggging, here it is
# used for debugging, in case i screw up the dataset while writing code
# idx_meta_genomecov <- read_delim("idx_meta_genomecov_PlatesAB_20180220.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
# if i need to read from a folder
# # idx_meta_genomecov <- read_delim("AB_outputs_20180214_F2308_f0x2_q60/idx_meta_genomecov_PlatesAB_20180214.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
```
...
# write_tsv(idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide, "idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide_PlatesAB_20180220.txt")
...
# uncomment to save table
# write_tsv(idx_meta_genomecov_mapped_reads_COI_lysis_corr_coefvarmin_wide, "idx_meta_genomecov_mapped_reads_COI_lysis_corr_coefvarmin_wide_PlatesAB_20180220.txt")
```
