...
idx %>% distinct(Full_name_of_the_sample) %>% arrange(Full_name_of_the_sample) # number of distinct values of samtools_filter: should be 176
...
filename2 <- "lysis_buffer_data_20161025.xlsx"
lysisbufferdata <- read_excel(file.path(folder3, filename2), sheet = "Orig_bwa", na = "NA") 

samplemetadata <- left_join(samplemetadata, lysisbufferdata) # R message: "Joining, by = c("Full_name_of_the_sample", "sample")"
...
colnames(temp) <- c("Date", "Plot", "Trap", "Week")
...
idx_meta_genomecov <- idx_meta_genomecov %>% dplyr::select(Date:mitogenome, COI_Species, mapped_reads, sample:pathname_genomecov)

names(idx_meta_genomecov)
#  [1] "Date"                       "Plot"                       "Trap"                       "Week"                      
#  [5] "mitogenome"                 "COI_Species"                "mapped_reads"               "sample"                    
#  [9] "well"                       "samtools_filter"            "pathname"                   "Full_name_of_the_sample"   
# [13] "Sample_alias"               "ArcDyn_Plate_name"          "EI_Plate_name"              "TGAC_barcode"              
# [17] "Concentration_ng_per_ul"    "Sample_volume_ul"           "Quantification_method"      "x.260_280_ratio"           
# [21] "lysis_buffer_purified_ml"   "lysis_buffer_orig_total_ml" "lysis_buffer_proportion"    "sum_coverage"              
# [25] "mean_coverage"              "stddev"                     "coefvar"                    "length"                    
# [29] "pathname_genomecov"  
# 
...
COI_spike_reads %>% distinct(Full_name_of_the_sample) %>% count()  # number of distinct values of Full_name_of_the_sample: should be 176
COI_spike_reads %>% distinct(Full_name_of_the_sample) %>% arrange(Full_name_of_the_sample) # number of distinct values of Full_name_of_the_sample: should be 176
...
COI_spike_reads_wide <- COI_spike_reads_wide %>% rename(Lepidoptera_Bombycidae_Bombyx_mori_10ng_COI="1-2_Lepidoptera_Bombycidae_Bombyx_mori_10ng_COI")

COI_spike_reads_wide <- COI_spike_reads_wide %>% rename(Coleoptera_Elateridae_40ng_COI="2-1_Coleoptera_Elateridae_40ng_COI")

COI_spike_reads_wide <- COI_spike_reads_wide %>% rename(Coleoptera_Mordellidae_20ng_COI="3-1_Coleoptera_Mordellidae_20ng_COI")
...
#  [7] "mapped_reads"                                "sample"                                     
#  [9] "well"                                        "samtools_filter"                            
# [11] "pathname"                                    "Full_name_of_the_sample"                    
# [13] "Sample_alias"                                "ArcDyn_Plate_name"                          
# [15] "EI_Plate_name"                               "TGAC_barcode"                               
# [17] "Concentration_ng_per_ul"                     "Sample_volume_ul"                           
# [19] "Quantification_method"                       "x.260_280_ratio"                            
# [21] "lysis_buffer_purified_ml"                    "lysis_buffer_orig_total_ml"                 
# [23] "lysis_buffer_proportion"                     "sum_coverage"                               
# [25] "mean_coverage"                               "stddev"                                     
# [27] "coefvar"                                     "length"                                     
...
# reorder columns
idx_meta_genomecov <- idx_meta_genomecov %>% dplyr::select(Date:mapped_reads, mapped_reads_COI_corr, mapped_reads_COI_lysis_corr, mean_coverage, stddev, coefvar, length, sample:COI_corr)  
