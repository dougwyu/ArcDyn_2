...
outputfolder <- "AB_outputs_20180527_F2308_f0x2_q48_minimap2"
idxmetagenomcovfile <- "idx_meta_genomecov_PlatesAB_20180527.txt"
...
outputfolder <- "A2B2_outputs_20180527_F2308_f0x2_q48_minimap2"
idxmetagenomcovfile <- "idx_meta_genomecov_PlatesA2B2_20180527.txt"
...
outputfolder <- "EF_outputs_20180527_F2308_f0x2_q48_minimap2"
idxmetagenomcovfile <- "idx_meta_genomecov_PlatesEF_20180527.txt"

idx_meta_genomecov_EF <- read_delim(file.path(outputfolder, idxmetagenomcovfile), delim = "\t", escape_double = FALSE, trim_ws = TRUE)
...
outputfolder <- "GH_outputs_20180527_F2308_f0x2_q48_minimap2"
idxmetagenomcovfile <- "idx_meta_genomecov_PlatesGH_20180527.txt"
...
# confirm 308 mitogenomes
idx_meta_genomecov_ABA2B2EFGH %>% distinct(mitogenome) %>% count()  # 308: number of distinct values of mitogenome (311 total mitogenomes - the 3 COI spikes = 308)
...
```{r add taxonomic ranks}
taxonomyfilepath <- "~/Dropbox/Working_docs/Roslin_Greenland/2017/bulk_samples/mapping_git/5_taxonomy/mitogenome_fasta"
taxonomyfile <- "Species_for_mitogenome_sequencing_20180721.xlsx"
...
# to_be_sanger_sequenced == "no" indicates specimens that successfully got contigs, either at EI or Copenhagen
...
idx_meta_genomecov_ABA2B2EFGH_wide_pctcovermin <- idx_meta_genomecov_ABA2B2EFGH %>% dplyr::filter(pct_coverage >= pctcovermin) %>% dplyr::select(DateTrapEIRUN, mitogenome, mapped_reads_COI_lysis_corr) %>% dplyr::arrange(DateTrapEIRUN) %>% tidyr::spread(DateTrapEIRUN, mapped_reads_COI_lysis_corr) %>% dplyr::arrange(mitogenome)

```


```{r write tables, eval=FALSE}
# idx_meta-genomecov long table for Otso, 217448 rows, 37 columns
# setwd("~/Dropbox/Working_docs/Roslin_Greenland/2017/bulk_samples/PlatesABA2B2EFGH")
# write_tsv(idx_meta_genomecov_ABA2B2EFGH, "idx_meta_genomecov_ABA2B2EFGH_minimap2_20180527.txt")
# # # idx_meta-genomecov wide table for visualisation:  308 rows, 707 columns
# write_tsv(idx_meta_genomecov_ABA2B2EFGH_wide, "idx_meta_genomecov_ABA2B2EFGH_wide_minimap2_20180527.txt")
# # # idx_meta-genomecov wide table for visualisation:  148 rows, 707 columns
# write_tsv(idx_meta_genomecov_ABA2B2EFGH_wide_pctcovermin, "idx_meta_genomecov_ABA2B2EFGH_wide_pctcovermin_minimap2_20180527.txt")
# #
```
...
idx_meta_genomecov_AB$DateTrapEIRUN <- str_c("Date", year(idx_meta_genomecov_AB$Date), str_pad(month(idx_meta_genomecov_AB$Date), 2, side = "left", pad = "0"), str_pad(day(idx_meta_genomecov_AB$Date), 2, side = "left", pad = "0"), idx_meta_genomecov_AB$Trap, str_replace(idx_meta_genomecov_AB$EI_RUN, "idx_meta_genomecov_", ""), sep = "_")

# Make wide table for visualisation (left to right should follow temporal order (DateTrapEIRUN))
idx_meta_genomecov_AB_wide_pctcovmin <- idx_meta_genomecov_AB %>% dplyr::filter(pct_coverage > 0.1) %>% dplyr::select(DateTrapEIRUN, mitogenome, mapped_reads_COI_lysis_corr) %>% dplyr::arrange(DateTrapEIRUN) %>% tidyr::spread(DateTrapEIRUN, mapped_reads_COI_lysis_corr) %>% dplyr::arrange(mitogenome)

# idx_meta-genomecov wide table for visualisation:  157 rows, 702 columns
# write_tsv(idx_meta_genomecov_AB_wide_pctcovmin, "idx_meta_genomecov_AB_wide_pctcovmin_minimap2_20180527.txt")
```

Summary tables to show how many reads and detected species per sample, also summed over year and trap, etc.

```{r summary tables}
setwd("~/Dropbox/Working_docs/Roslin_Greenland/2017/bulk_samples/PlatesABA2B2EFGH/minimap2_308mitogenomes_20180527")
idx_meta_genomecov <- read_tsv("idx_meta_genomecov_ABA2B2EFGH_minimap2_20180527.txt")
...
sum_idx_meta_genomecov <- idx_meta_genomecov %>% 
    ...
    arrange(Date, Trap)
# write_tsv(sum_idx_meta_genomecov, "idx_meta_genomecov_ABA2B2EFGH_summ0_minimap2_20180527.txt")
...
sum_idx_meta_genomecov <- idx_meta_genomecov %>% 
    ...
    arrange(Date, Trap)
# write_tsv(sum_idx_meta_genomecov, "idx_meta_genomecov_ABA2B2EFGH_summ1_minimap2_20180527.txt")
...
sum_idx_meta_genomecov2 <- idx_meta_genomecov %>% 
    ...
    arrange(Date, Trap)
# write_tsv(sum_idx_meta_genomecov2, "idx_meta_genomecov_ABA2B2EFGH_summ2_minimap2_20180527.txt")
...
sum_idx_meta_genomecov3 <- idx_meta_genomecov %>% 
    ...
    arrange(Year, Trap)
# write_tsv(sum_idx_meta_genomecov3, "idx_meta_genomecov_ABA2B2EFGH_summ3_minimap2_20180527.txt")
