...
idxstatsgenomecovfolder <- paste0("outputs_PlatesA2B2_", samtoolsfilter, "_", samtoolsqual, "_minimap2_outputs_20181111")  # this is the enclosing folder around the idxstats and genomcov files for a given set of soups
...
idx_mitogenome_table <- idx %>% group_by(well) %>% distinct(mitogenome) %>% count() %>% arrange(desc(n)); View(idx_mitogenome_table) # all columns should be 352
...
idx_meta %>% distinct(mitogenome) %>% count()  # number of distinct values of mitogenome (incl 3 COI spikes):  352
...
# saveRDS(genomecoverages_summ, "genomecoverages_summ_PlatesA2B2_20181111.RDS", compress = TRUE)
...
# filter out samples with COI_spike_sum < 282 832 (summed over all 160 mitogenomes in the sample).  The largest value of elaterid_mordellid_sum is 16,727,485,975, so this keeps the COI_corr from getting too large (and thus causing the COI_corr reads from getting too small)
...
# sanity check:  161 samples, which is correct since i removed 7 samples
...
# write_tsv(idx_meta_genomecov, "idx_meta_genomecov_PlatesA2B2_20181111.txt") # so if i need to read in a file during debuggging, here it is
...
# write_tsv(idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide, "idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide_PlatesA2B2_20181111.txt")
...
# uncomment to save table
write_tsv(idx_meta_genomecov_mapped_reads_COI_lysis_corr_pctcovmin_wide, "idx_meta_genomecov_mapped_reads_COI_lysis_corr_pctcovmin_wide_PlatesA2B2_20181111.txt")
```
