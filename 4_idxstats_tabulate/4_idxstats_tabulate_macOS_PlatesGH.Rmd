...
idxstatsgenomecovfolder <- paste0("outputs_PlatesGH_", samtoolsfilter, "_", samtoolsqual, "_minimap2_outputs_20181111")  # this is the enclosing folder around the idxstats and genomcov files for a given set of soups
...
```{r data files for Otso on 20181221}
...
# soups dataset
soupdata <- read_tsv("send_to_ArcDyn_collaborators_20180721/idx_meta_genomecov_ABA2B2EFGH_minimap2_20180721.txt.zip")
...
Lepidoptera_Bombycidae_Bombyx_mori_10ng_COI <- rep(10, nrow(soupdata))
Coleoptera_Mordellidae_20ng_COI <- rep(20, nrow(soupdata))
Coleoptera_Elateridae_40ng_COI <- rep(40, nrow(soupdata))
...
env_design <- bind_cols(env_design, data_frame(Lepidoptera_Bombycidae_Bombyx_mori_10ng_COI, Coleoptera_Mordellidae_20ng_COI, Coleoptera_Elateridae_40ng_COI))

# env_data
# make long-format dataset of the COI_spike mapped reads, one row per sample and spike_species
env_spikes <- soupdata %>% dplyr::select(sample = DateTrapEIRUN, Lepidoptera_Bombycidae_Bombyx_mori_10ng_COI, Coleoptera_Mordellidae_20ng_COI, Coleoptera_Elateridae_40ng_COI) %>% 
    tidyr::gather(species, mapped_reads_COI, Lepidoptera_Bombycidae_Bombyx_mori_10ng_COI, Coleoptera_Mordellidae_20ng_COI, Coleoptera_Elateridae_40ng_COI) %>% 
    dplyr::distinct(sample, species, mapped_reads_COI)
...
# code if i want to create a column indicating mitogenome or COI_spike
# env_data$mito_spike <- if_else(grepl("ng_COI", env_data$species), "spike", "mitogenome") %>% dplyr::select(sample, mito_spike, species, mapped_reads, pct_coverage)

# save(mitogenomes_data, env_design, env_data, file = "~/Dropbox/Working_docs/Roslin_Greenland/2018/manuscript_methods/Supp_Info_data_and_R_scripts/arcdyn_suppfiles_soups.RData")
...
### extract COI_spike columns, make a long-format dataset (with gather), change column name to mitogenome, reduce to unique COI_spike names, and add a new column with index numbers (1,2,3)
mock_spikes <- mockall %>% 
    dplyr::select(Lepidoptera_Bombycidae_Bombyx_mori_10ng_COI, Coleoptera_Mordellidae_20ng_COI, Coleoptera_Elateridae_40ng_COI) %>% 
    tidyr::gather(species, mapped_reads, Lepidoptera_Bombycidae_Bombyx_mori_10ng_COI, Coleoptera_Mordellidae_20ng_COI, Coleoptera_Elateridae_40ng_COI) %>% 
...
mock_inputspp$species <- if_else(grepl("ng_COI", mock_inputspp$mitogenome), "spike", "sp")
mock_inputspp <- mock_inputspp %>% 
    unite(species, c("species", "order"), sep = "") %>% 
    dplyr::select(species, mitogenome)
...
mock_spikes <- mockall %>% dplyr::select(plate = Plate, shortname = Expt_shortname, experiment, run = Replicate, Lepidoptera_Bombycidae_Bombyx_mori_10ng_COI, Coleoptera_Mordellidae_20ng_COI, Coleoptera_Elateridae_40ng_COI) %>% 
    tidyr::gather(mitogenome_spike, mapped_reads_COI, Lepidoptera_Bombycidae_Bombyx_mori_10ng_COI, Coleoptera_Mordellidae_20ng_COI, Coleoptera_Elateridae_40ng_COI) %>% 
...
mock_spikes <- mock_spikes %>% mutate(
    input_amount = case_when(
        mitogenome_spike == "Lepidoptera_Bombycidae_Bombyx_mori_10ng_COI" ~ 10,
        mitogenome_spike == "Coleoptera_Mordellidae_20ng_COI" ~ 20,
        mitogenome_spike == "Coleoptera_Elateridae_40ng_COI" ~ 40
    )
)
...
### select columns except for the COI_spike columns
mock_data <- mockall %>% dplyr::select(plate = Plate, shortname = Expt_shortname, experiment, run = Replicate, mitogenome_spike = mitogenome, mapped_reads_mitogenome = mapped_reads, pct_coverage_mitogenome = pct_coverage)
...
## save data files
# save(mock_inputspp, mock_design, mock_data, file = "~/Dropbox/Working_docs/Roslin_Greenland/2018/manuscript_methods/Supp_Info_data_and_R_scripts/arcdyn_suppfiles_mocks.RData")



# load("~/Dropbox/Working_docs/Roslin_Greenland/2018/manuscript_methods/Supp_Info_data_and_R_scripts/arcdyn_suppfiles_mocks.RData")
# load("~/Dropbox/Working_docs/Roslin_Greenland/2018/manuscript_methods/Supp_Info_data_and_R_scripts/arcdyn_suppfiles_soups.RData")
```
