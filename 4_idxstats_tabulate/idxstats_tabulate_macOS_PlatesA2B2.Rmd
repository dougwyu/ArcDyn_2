...
idx %>% distinct(sample) %>% count()  # number of distinct values of sample:  168  I originally input 192, but only have 176 idx files.  
idx %>% distinct(pathname) %>% count()  # number of distinct values of sample:  168  I originally input 192, but only have 176 idx files.  
...
26880/160 # = 168 # 26880 rows / 160 mitogenomes = 168 samples
...
# saveRDS(genomecoverages_summ, "genomecoverages_summ_PlatesA2B2_20180209.RDS", compress = TRUE)
# genomecoverages_summ <- readRDS("genomecoverages_summ_PlatesA2B2_20180209.RDS")
...
# write_tsv(idx_meta_genomecov, "idx_meta_genomecov_PlatesAB_20180209.txt") # so if i need to read in a file during debuggging, here it is
...
# idx_meta_genomecov <- read_delim("idx_meta_genomecov_PlatesAB_20180209.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
...
# filter out samples with COI_spike_sum < 800 000 (summed over all 160 mitogenomes in the sample)ß.  The largest value of elaterid_mordellid_sum is 7,922,133,493, so this keeps the COI_corr from getting too large (and thus causing the COI_corr reads from getting too small)
...
# write_tsv(idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide, "idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide_PlatesA2B2_20180209.txt")
```
