...
idxstatsgenomecovfolder <- "minimap2_all_outputs_PlatesA2B2"  # this is the enclosing folder around the idxstats and genomcov files for a given set of soups
...
head(idx_files)
    # Sys.glob is used to generate:  
        # [1] "minimap2_all_outputs_PlatesA2B2/Sample_PRO1322_PlateA_A1_F2308_f0x2_q1_sorted.bam_idxstats.txt" 
...
idx_mitogenome_table <- idx %>% group_by(well) %>% distinct(mitogenome) %>% count() %>% arrange(desc(n)) # all columns should be 160
...
head(genomecov_files)
    # Sys.glob is used to generate pathnames:  
        # [1] "minimap2_all_outputs_PlatesA2B2/Sample_PRO1322_PlateA_A1_F2308_f0x2_q1_sorted_genomecov_d.txt.gz" 
...
# saveRDS(genomecoverages_summ, "genomecoverages_summ_PlatesA2B2_20180211.RDS", compress = TRUE)
# genomecoverages_summ <- readRDS("genomecoverages_summ_PlatesA2B2_20180211.RDS")
...
# write_tsv(idx_meta_genomecov, "idx_meta_genomecov_PlatesA2B2_20180211.txt") # so if i need to read in a file during debuggging, here it is
# used for debugging, in case i screw up the dataset while writing code
# idx_meta_genomecov <- read_delim("idx_meta_genomecov_PlatesA2B2_20180211.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
# 
```
...
# write_tsv(idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide, "idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide_PlatesA2B2_20180211.txt")
```
