...
idxstatsgenomecovfolder <- paste0("minimap2_all_outputs_PlatesA2B2_", samtoolsfilter, "_", samtoolsqual)  # this is the enclosing folder around the idxstats and genomcov files for a given set of soups

# use this version for COI_barcodes mapping
# idxstatsgenomecovfolder <- paste0("minimap2_all_outputs_PlatesA2B2_", samtoolsfilter, "_COIBarcodesOnly")  # this is the enclosing folder around the idxstats and genomcov files for a given set of soups
...
samplemetadatafolder <- "~/Dropbox/Working_docs/Roslin_Greenland/2016/bulk_samples/PlatesAB_EI_20160512/sample_tracking"  # this is the enclosing folder around the sample metadata spreadsheet, which i will place on hpc
...
idx %>% distinct(sample) %>% count()  # number of distinct values of sample:  168  I originally input 192, but only have 168 idx files.  
idx %>% distinct(pathname) %>% count()  # number of distinct values of sample:  168  I originally input 192, but only have 168 idx files.  
...
idx_mitogenome_table <- idx %>% group_by(well) %>% distinct(mitogenome) %>% count() %>% arrange(desc(n)); View(idx_mitogenome_table) # all columns should be 160
length(idx$mitogenome) / idx_mitogenome_table[1,2] == idx %>% distinct(sample) %>% count()  # should evaluate to TRUE
# 26880 rows / 160 mitogenomes = 168 samples
```
...
The Earlham Institute fastq filenames for PlatesA2B2 are different from PlatesAB. In PlatesA2B2, they use ArcDyn's sample names:  PlatesAB, no longer using EI names, e.g. IPO3916, IP3917.  So i have to do some formatting of the samplemetadata to produce sample names in the format:  PlateA_A1, ...
...
# [17] "EI_sample"                  "TGAC_barcode"               "Concentration_ng_per_ul"    "Sample_volume_ul"          
...
```{r sanity checks}
idx_meta %>% distinct(Date) %>% count()  # number of distinct values of Date:  67
...
idx_meta %>% distinct(sample) %>% count()  # number of distinct values of sample:   168 (168 is the number of input idx files)
idx_meta %>% distinct(sample)
idx_meta %>% distinct(Sample_alias) %>% count()  # number of distinct values of Sample_alias:  should be = sample = 168
...
idx_meta %>% distinct(Full_name_of_the_sample) %>% count()  # number of distinct values of Full_name_of_the_sample: should be 168
idx_meta %>% distinct(Full_name_of_the_sample) %>% arrange(Full_name_of_the_sample) # number of distinct values of samtools_filter: should be 168
```
...
# saveRDS(genomecoverages_summ, "genomecoverages_summ_PlatesA2B2_20180220.RDS", compress = TRUE)
# genomecoverages_summ <- readRDS("genomecoverages_summ_PlatesA2B2_20180220.RDS")
...
# [17] "EI_Plate_name"              "EI_sample"                  "TGAC_barcode"               "Concentration_ng_per_ul"   
...
COI_spike_reads %>% distinct(Full_name_of_the_sample) %>% count()  # number of distinct values of Full_name_of_the_sample: should be 168
COI_spike_reads %>% distinct(Full_name_of_the_sample) %>% arrange(Full_name_of_the_sample) # number of distinct values of Full_name_of_the_sample: should be 168
...
# filter out samples with COI_spike_sum < 800 000 (summed over all 160 mitogenomes in the sample).  The largest value of elaterid_mordellid_sum is 7,922,133,493, so this keeps the COI_corr from getting too large (and thus causing the COI_corr reads from getting too small)

sample_names <- sum_idx_meta_genomecov %>% dplyr::filter(COI_spike_sum < 800000) %>% dplyr::select(Full_name_of_the_sample)
samples_to_remove <- sample_names %>% dplyr::pull(Full_name_of_the_sample) # pull out a single variable
samples_to_remove # 15 samples to remove
idx_meta_genomecov <- idx_meta_genomecov %>% filter(!Full_name_of_the_sample %in% samples_to_remove) # 24021 obs

# sanity check:  153 samples, which is correct since i removed 15 samples
...
# should be 2, and it's 1.87, sd = 0.26
...
# should be 4, and its 3.41, sd = 0.70
...
# should be 2, and it's 1.83, sd = 0.14
...
# write_tsv(idx_meta_genomecov, "idx_meta_genomecov_PlatesA2B2_20180220.txt") # so if i need to read in a file during debuggging, here it is
# used for debugging, in case i screw up the dataset while writing code
# idx_meta_genomecov <- read_delim("idx_meta_genomecov_PlatesA2B2_20180220.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
#
```
...
# write_tsv(idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide, "idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide_PlatesA2B2_20180220.txt")
...
# mapped_reads_COI_lysis_corr_coefvarmin # set read number = 0 if coefvar > coefvarmin (e.g. 1.8)
...
# uncomment to save table
# write_tsv(idx_meta_genomecov_mapped_reads_COI_lysis_corr_coefvarmin_wide, "idx_meta_genomecov_mapped_reads_COI_lysis_corr_coefvarmin_wide_PlatesA2B2_20180220.txt")
```
