...
samtoolsqual <- "q48"
idxstatsgenomecovfolder <- paste0("bwa_outputs_PlatesA2B2_", samtoolsfilter, "_", samtoolsqual)  # this is the enclosing folder around the idxstats and genomcov files for a given set of soups
...
# sanity checks
idx %>% distinct(well) %>% count()  # 92, number of distinct values of well, as a sanity check
idx %>% distinct(sample) %>% count()  # number of distinct values of sample:  152  I originally input 192, but only have 168 idx files.  
idx %>% distinct(pathname) %>% count()  # number of distinct values of sample:  152  I originally input 192, but only have 168 idx files.  
...
idx_mitogenome_table <- idx %>% group_by(well) %>% distinct(mitogenome) %>% count() %>% arrange(desc(n)); View(idx_mitogenome_table) # all columns should be 308
length(idx$mitogenome) / idx_mitogenome_table[1,2] == idx %>% distinct(sample) %>% count()  # should evaluate to TRUE
# 46816 rows / 308 mitogenomes = 152 samples
```
...
```{r sanity checks}
idx_meta %>% distinct(Date) %>% count()  # number of distinct values of Date:  65
...
idx_meta %>% distinct(mitogenome) %>% count()  # number of distinct values of mitogenome (incl 3 COI spikes):  308
idx_meta %>% distinct(sample) %>% count()  # number of distinct values of sample:   152 (168 is the number of input idx files)
...
idx_meta %>% distinct(well) %>% count()  # number of distinct values of well:  92
