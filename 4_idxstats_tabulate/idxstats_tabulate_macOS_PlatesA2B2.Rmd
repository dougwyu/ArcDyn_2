...
# saveRDS(genomecoverages_summ, "genomecoverages_summ_PlatesA2B2_20180226.RDS", compress = TRUE)
# genomecoverages_summ <- readRDS("genomecoverages_summ_PlatesA2B2_20180226.RDS")
...
# filter out samples with COI_spike_sum < 800 000 (summed over all 160 mitogenomes in the sample).  The largest value of elaterid_mordellid_sum is 16,727,485,975, so this keeps the COI_corr from getting too large (and thus causing the COI_corr reads from getting too small)
...
samples_to_remove # 11 samples to remove
idx_meta_genomecov <- idx_meta_genomecov %>% filter(!Full_name_of_the_sample %in% samples_to_remove) # 24021 obs

# sanity check:  157 samples, which is correct since i removed 11 samples
...
# write_tsv(idx_meta_genomecov, "idx_meta_genomecov_PlatesA2B2_20180227.txt") # so if i need to read in a file during debuggging, here it is
# used for debugging, in case i screw up the dataset while writing code
# idx_meta_genomecov <- read_delim("idx_meta_genomecov_PlatesA2B2_20180223.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
#
```
...
# write_tsv(idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide, "idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide_PlatesA2B2_20180227.txt")
...
# uncomment to save table
# write_tsv(idx_meta_genomecov_mapped_reads_COI_lysis_corr_coefvarmin_wide, "idx_meta_genomecov_mapped_reads_COI_lysis_corr_coefvarmin_wide_PlatesA2B2_20180227.txt")
```
