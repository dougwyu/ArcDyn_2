...
idxstatsgenomecovfolder <- "minimap2_all_outputs_PlatesEF"  # this is the enclosing folder around the idxstats and genomcov files for a given set of soups
...
idxstatsfile <- "*F2308_q1_sorted.bam_idxstats\\.txt"  # choose the filename suffix to match the desired samtools filter. precede with * as a wildcard. \\ is used to escape the . before txt (because it is read with grep as "any character" in list.files() below)
...
head(idx_files)
    # Sys.glob is used to generate:  
        # [1] "minimap2_all_outputs_PlatesEF/PlateE_A1_AACCAACCG-GCCAAT_F2308_f0x2_q1_sorted.bam_idxstats.txt" 
...
genomecovfile <- "*F2308_q1_sorted_genomecov_d\\.txt\\.gz"  # choose the filename pattern to match the desired samtools filter. precede with * as a wildcard for the sample names (e.g. Sample_IPO3916_A10_).      
# \\ is used to escape the . before txt (because it is read with grep as "any character" in list.files()
...
head(genomecov_files)
    # Sys.glob is used to generate pathnames:  
        # [1] "minimap2_all_outputs_PlatesEF/PlateE_A1_AACCAACCG-GCCAAT_F2308_f0x2_q1_sorted_genomecov_d.txt.gz" 
...
# saveRDS(genomecoverages_summ, "genomecoverages_summ_PlatesEF_20180211.RDS", compress = TRUE)
# genomecoverages_summ <- readRDS("genomecoverages_summ_PlatesEF_20180211.RDS")
...
samples_to_remove # 2 samples to remove
idx_meta_genomecov <- idx_meta_genomecov %>% filter(!Full_name_of_the_sample %in% samples_to_remove) # 29830 obs

# sanity check:  190 samples, which is correct since i removed 2 samples
...
# should be 2, and it's 1.47, sd = 0.24
...
idx_meta_genomecov <- idx_meta_genomecov %>% dplyr::mutate(COI_corr = bombycid_mordellid_sum/min(bombycid_mordellid_sum))
...
sum_idx_meta_genomecov2 <- idx_meta_genomecov %>% 
    group_by(Full_name_of_the_sample) %>% 
    summarise_at(vars(mapped_reads, mapped_reads_COI_corr, mapped_reads_COI_lysis_corr, sum_coverage, bombycid_mordellid_sum), funs(sum(as.numeric(.)))) %>%
...
# [41] "ratio_elaterid40_mordellid20"                "bombycid_mordellid_sum"                     
# [43] "COI_corr"     
```
...
mocks_idx_meta_genomecov <- idx_meta_genomecov %>% dplyr::filter(Plot == "evenDNA")
...
# write_tsv(idx_meta_genomecov, "idx_meta_genomecov_PlatesEF_20180211.txt") # so if i need to read in a file during debuggging, here it is
# used for debugging, in case i screw up the dataset while writing code
# idx_meta_genomecov <- read_delim("idx_meta_genomecov_PlatesEF_20180211.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
...
# write_tsv(idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide, "idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide_PlatesEF_20180211.txt")
# 
# write_tsv(mocks_idx_meta_genomecov_mapped_reads_wide, "mocks_idx_meta_genomecov_mapped_reads_wide_PlatesEF_20180211.txt")
# 
# write_tsv(mocks_idx_meta_genomecov_mapped_reads_COI_corr_wide, "mocks_idx_meta_genomecov_mapped_reads_COI_corr_wide_PlatesEF_20180211.txt")
```
