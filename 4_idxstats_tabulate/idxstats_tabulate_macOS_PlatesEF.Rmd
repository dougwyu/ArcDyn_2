...
# use this version for bwa mapping
idxstatsgenomecovfolder <- paste0("bwa_outputs_PlatesEF_", samtoolsfilter, "_", samtoolsqual)  # this is the enclosing folder around the idxstats and genomcov files for a given set of soups

# use this version for minimap2 mapping
# idxstatsgenomecovfolder <- paste0("minimap2_all_outputs_PlatesEF_", samtoolsfilter, "_", samtoolsqual)  # this is the enclosing folder around the idxstats and genomcov files for a given set of soups

# use this version for COI_barcodes mapping
# idxstatsgenomecovfolder <- paste0("minimap2_all_outputs_PlatesEF_", samtoolsfilter, "_COIBarcodesOnly")  # this is the enclosing folder around the idxstats and genomcov files for a given set of soups
...
samplemetadatafolder <- "~/Dropbox/Working_docs/Roslin_Greenland/2017/bulk_samples/PlatesEF"  # this is the enclosing folder around the sample metadata spreadsheet, which in this case is the same as the working directory overall
...
idx_mitogenome_table <- idx %>% group_by(well) %>% distinct(mitogenome) %>% count() %>% arrange(desc(n)); View(idx_mitogenome_table) # should be 160 in all rows

length(idx$mitogenome) / idx_mitogenome_table[1,2] == idx %>% distinct(sample) %>% count()  # should evaluate to TRUE
# 30720 rows / 160 mitogenomes = 192 samples
```
...
#  [1] "Full_name_of_the_sample"  "Date"                     "Short_name_of_the_sample" "Sample_alias"            
#  [5] "ArcDyn_Plate_name"        "EI_Plate_name"            "well"                     "DNA_conc"                
#  [9] "species_name"             "barcode_used"             "Sample_type"              "sample_reference"        
# [13] "Concentration_ng_per_ul"  "Sample_volume_ul"         "Quantification_method"    "x.260_280_ratio"         
# [17] "x.260_230_ratio"          "Buffer_used"              "Extraction_kit_protocol" 
```
...
# [13] "Year"                       "Sample_alias"               "ArcDyn_Plate_name"          "EI_Plate_name"             
# [17] "DNA_conc"                   "sample_reference"           "Concentration_ng_per_ul"    "Sample_volume_ul"          
# [21] "Quantification_method"      "x.260_280_ratio"            "lysis_buffer_orig_total_ul" "lysis_buffer_purified_ul"  
...
# saveRDS(genomecoverages_summ, "genomecoverages_summ_PlatesEF_20180227.RDS", compress = TRUE)
# genomecoverages_summ <- readRDS("EF_outputs_20180220_F2308_q48/genomecoverages_summ_PlatesEF_20180220.RDS")
...
#  [1] "Date"                       "Plot"                       "Trap"                       "Week"                      
#  [5] "mitogenome"                 "COI_Species"                "mapped_reads"               "mt_length"                 
#  [9] "sample"                     "well"                       "samtools_filter"            "pathname"                  
# [13] "Full_name_of_the_sample"    "Year"                       "Sample_alias"               "ArcDyn_Plate_name"         
# [17] "EI_Plate_name"              "DNA_conc"                   "sample_reference"           "Concentration_ng_per_ul"   
# [21] "Sample_volume_ul"           "Quantification_method"      "x.260_280_ratio"            "lysis_buffer_orig_total_ul"
# [25] "lysis_buffer_purified_ul"   "lysis_buffer_proportion"    "sum_coverage"               "mean_coverage"             
# [29] "stddev"                     "coefvar"                    "length"                     "pathname_genomecov"   
```
...
# [29] "stddev"                                      "coefvar"                                    
# [31] "length"                                      "pathname_genomecov"                         
# [33] "Lepidoptera_Bombycidae_Bombyx_mori_10ng_COI" "Coleoptera_Elateridae_40ng_COI"             
# [35] "Coleoptera_Mordellidae_20ng_COI"      
```
...
# filter out samples with COI_spike_sum < 100 000 (summed over all 157 mitogenomes in the sample).  This removes 2 samples with very small numbers of COI_spike_sum

sample_names <- sum_idx_meta_genomecov %>% dplyr::filter(COI_spike_sum < 500000) %>% dplyr::select(Full_name_of_the_sample)
samples_to_remove <- sample_names %>% dplyr::pull(Full_name_of_the_sample) # pull out a single variable
samples_to_remove # 24 samples to remove (using q48)
idx_meta_genomecov <- idx_meta_genomecov %>% filter(!Full_name_of_the_sample %in% samples_to_remove) # 51576 obs

# sanity check:  168 samples, which is correct since i removed 24 samples
...
# should be 2, and it's 2.18, sd = 0.37
...
# should be 4, and its 3.22, sd = 0.79
...
# should be 2, and it's 1.48, sd = 0.24

# Although the ratio_mordellid20_bombyx10 is closer to its expected value, for now we use the sum of Coleoptera_Elateridae_40ng_COI and Coleoptera_Mordellidae_20ng_COI to correct reads, to allow for consistency with the other libraries
...
# not using this
# # Thus, use sum of Lepidoptera_Bombycidae_Bombyx_mori_10ng_COI and Coleoptera_Mordellidae_20ng_COI to correct reads
# # calculate sum of these two COI spike reads for each row
# idx_meta_genomecov <- idx_meta_genomecov %>% dplyr::mutate(bombycid_mordellid_sum = Lepidoptera_Bombycidae_Bombyx_mori_10ng_COI + Coleoptera_Mordellidae_20ng_COI)
# ggplot(idx_meta_genomecov, aes(x=bombycid_mordellid_sum)) + geom_density(alpha=0.5)
```
...
# write_tsv(mocks_idx_meta_genomecov, "mocks_idx_meta_genomecov_PlatesEF_20180220.txt") # so if i need to read in a file during debuggging, here it is
# mocks_idx_meta_genomecov <- read_delim("mocks_idx_meta_genomecov_PlatesEF_20180220.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
...
# write_tsv(idx_meta_genomecov, "idx_meta_genomecov_PlatesEF_20180227.txt") # so if i need to read in a file during debuggging, here it is
# used for debugging, in case i screw up the dataset while writing code
# idx_meta_genomecov <- read_delim("idx_meta_genomecov_PlatesEF_20180220.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
...
# write_tsv(idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide, "idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide_PlatesEF_20180227.txt")
...
# mapped_reads_COI_lysis_corr_coefvarmin # read number = 0 if coefvar > coefvarmin (e.g. 1.8)
idx_meta_genomecov_mapped_reads_COI_lysis_corr_coefvarmin_wide <- idx_meta_genomecov %>% dplyr::select(DateTrap, mitogenome, mapped_reads_COI_lysis_corr_coefvarmin) %>% dplyr::arrange(DateTrap) %>% tidyr::spread(DateTrap, mapped_reads_COI_lysis_corr_coefvarmin) %>% dplyr::arrange(mitogenome)

# uncomment to save table
# write_tsv(idx_meta_genomecov_mapped_reads_COI_lysis_corr_coefvarmin_wide, "idx_meta_genomecov_mapped_reads_COI_lysis_corr_coefvarmin_wide_PlatesEF_20180227.txt")
...
mocks_idx_meta_genomecov_coefvar_wide <- mocks_idx_meta_genomecov %>% dplyr::select(Sample_alias, mitogenome, coefvar) %>% dplyr::arrange(Sample_alias) %>% tidyr::spread(Sample_alias, coefvar) %>% dplyr::arrange(mitogenome)
...
# write_tsv(mocks_idx_meta_genomecov_mapped_reads_COI_corr_wide, "mocks_idx_meta_genomecov_mapped_reads_COI_corr_wide_PlatesEF_20180227.txt")
# # # 
# write_tsv(mocks_idx_meta_genomecov_coefvar_wide, "mocks_idx_meta_genomecov_coefvar_wide_PlatesEF_20180227.txt")
# # # 
# write_tsv(mocks_idx_meta_genomecov_mapped_reads_wide, "mocks_idx_meta_genomecov_mapped_reads_wide_PlatesEF_20180227.txt")#
```
