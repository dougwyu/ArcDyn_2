...
getwd()
folder1 <- "minimap2_all_outputs_PlatesAB"  # this is the enclosing folder around the idxstats and genomcov files for a given set of soups
...
# idx_files <- list.files(Sys.glob(file.path(folder1, "BWA??", folder2)), pattern = idxstatsfile, full.names = TRUE) # old version
idx_files <- list.files(Sys.glob(file.path(folder1)), pattern = idxstatsfile, full.names = TRUE)
head(idx_files)
...
idx_meta <- idx_meta %>% tidyr::separate(Full_name_of_the_sample, c("Date", "Plot", "Trap", "Week"), sep = "_", remove = FALSE)
idx_meta$Date <- as_date(ymd(idx_meta$Date))
...
# genomecov_files <- list.files(Sys.glob(file.path(folder1, "BWA??", folder2)), pattern = genomecovfile, full.names = TRUE) # previous version

genomecov_files <- list.files(Sys.glob(file.path(folder1)), pattern = genomecovfile, full.names = TRUE)
...
samples_to_remove <- sample_names %>% dplyr::pull(Full_name_of_the_sample)
