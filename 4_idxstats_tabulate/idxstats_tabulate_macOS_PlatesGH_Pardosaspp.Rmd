...
idxstatsgenomecovfolder <- paste0("pardosa_spp_outputs_PlatesGH_", samtoolsfilter, "_", samtoolsqual)  # this is the enclosing folder around the idxstats and genomcov files for a given set of soups
...
idx_mitogenome_table <- idx %>% group_by(well) %>% distinct(mitogenome) %>% count() %>% arrange(desc(n)); View(idx_mitogenome_table) # should be 310
...
```{r 2. ORIGINAL read sample metadata and create Short_name_of_the_sample, eval = FALSE, include = FALSE}
# This version was used when the samplemetadata only had year and week number, which i parsed and converted to a date (1st date of each week)
samplemetadata <- read_excel(file.path(samplemetadatafolder, samplemetadatafilename), sheet = "edited", na = "NA")
...
idx_meta %>% distinct(mitogenome) %>% count()  # number of distinct values of mitogenome (incl 3 COI spikes):  310
...
# saveRDS(genomecoverages_summ, "genomecoverages_summ_PlatesGH_pardosaspp_20180317.RDS", compress = TRUE)
# genomecoverages_summ <- readRDS("GH_outputs_20180227_F2308_q48/genomecoverages_summ_PlatesGH_20180227.RDS")
# 
...
# filter out samples with COI_spike_sum < 1 000 000.  This removes 13 samples with small numbers of COI_spike_sum including the small ME50_1 mock community which seems to have failed

sample_names <- sum_idx_meta_genomecov %>% dplyr::filter(COI_spike_sum < 1000000) %>% dplyr::select(Short_name_of_the_sample)
samples_to_remove <- sample_names %>% dplyr::pull(Short_name_of_the_sample) # pull out a single variable
samples_to_remove # 13 samples to remove including ME50_1
idx_meta_genomecov <- idx_meta_genomecov %>% filter(!Short_name_of_the_sample %in% samples_to_remove) # 29045 obs

# sanity check:  177 samples, which is correct since i removed 177 samples
...
# write_tsv(mocks_idx_meta_genomecov, "mocks_idx_meta_genomecov_PlatesGH_20180227.txt") # so if i need to read in a file during debuggging, here it is
# mocks_idx_meta_genomecov <- read_delim("mocks_idx_meta_genomecov_PlatesGH_20180227.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
...
# write_tsv(idx_meta_genomecov, "idx_meta_genomecov_PlatesGH_20180227.txt") # so if i need to read in a file during debuggging, here it is
# used for debugging, in case i screw up the dataset while writing code
# idx_meta_genomecov <- read_delim("idx_meta_genomecov_PlatesGH_20180227.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
```
...
# write_tsv(idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide, "idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide_PlatesGH_20180227.txt")
...
# write_tsv(idx_meta_genomecov_mapped_reads_COI_lysis_corr_coefvarmin_wide, "idx_meta_genomecov_mapped_reads_COI_lysis_corr_coefvarmin_wide_PlatesGH_20180227.txt")
...
# write_tsv(mocks_idx_meta_genomecov_mapped_reads_COI_corr_wide, "mocks_idx_meta_genomecov_mapped_reads_COI_corr_wide_PlatesGH_pardosaspp_20180317.txt")
# 
# write_tsv(mocks_idx_meta_genomecov_coefvar_wide, "mocks_idx_meta_genomecov_coefvar_wide_PlatesGH_pardosaspp_20180317.txt")
# 
# write_tsv(mocks_idx_meta_genomecov_mapped_reads_wide, "mocks_idx_meta_genomecov_mapped_reads_wide_PlatesGH_pardosaspp_20180317.txt")
```
