...
idxstatsgenomecovfolder <- "minimap2_all_outputs_PlatesGH"  # this is the enclosing folder around the idxstats and genomcov files for a given set of soups
...
head(idx_files)
    # Sys.glob is used to generate:  
        # [1] "minimap2_all_outputs_PlatesGH/Sample_PRO1747_PlateG_A1_F2308_f0x2_q1_sorted.bam_idxstats.txt"
...
idx %>% distinct(samtools_filter) # should look like:  F2308_f0x2_q1

idx_mitogenome_table <- idx %>% group_by(well) %>% distinct(mitogenome) %>% count()  # should be 160 for all wells (rows)

30400/160 # = 192 # 30720 rows / 160 mitogenomes = 168 samples
...
# samplemetadata$Year <- as_date(year(samplemetadata$Year))
# samplemetadata$Year <- year(samplemetadata$Date)
# samplemetadata <- samplemetadata %>% dplyr::select(Short_name_of_the_sample, Year, Plot:Extraction_kit_protocol_used)
...
#  [9] "well"                       "samtools_filter"            "pathname"                   "Short_name_of_the_sample"  
# [13] "Plate"                      "mock_ArcDyn"                "Sample_alias"               "EI_sample_name"            
# [17] "Concentration_ng_ul"        "Sample_volume_ul"           "qty_loaded_uL"              "Quantification_method"     
# [21] "x.260_280_ratio"            "lysis_buffer_orig_total_ul" "lysis_buffer_purified_ul"   "lysis_buffer_proportion"  
```
...
# idx_meta %>% distinct(Short_name_of_the_sample) %>% arrange(Short_name_of_the_sample) # number of distinct values of samtools_filter: should be 190
```
...
head(genomecov_files)
    # Sys.glob is used to generate pathnames:  
    # [1] "minimap2_all_outputs_PlatesGH/PlateE_A1_AACCAACCG-GCCAAT_F2308_f0x2_q1_sorted_genomecov_d.txt.gz" 
...
# saveRDS(genomecoverages_summ, "genomecoverages_summ_PlatesGH_20180209.RDS", compress = TRUE)
# genomecoverages_summ <- readRDS("genomecoverages_summ_PlatesGH_20180209.RDS")
# 
...
idx_meta_genomecov <- idx_meta_genomecov %>% arrange(Year, Week, Trap)
...
idx_meta_genomecov <- idx_meta_genomecov %>% dplyr::select(Short_name_of_the_sample, Year:mitogenome, COI_Species, mock_ArcDyn, mapped_reads, mt_length:pathname_genomecov)

names(idx_meta_genomecov)
#  [1] "Short_name_of_the_sample"   "Year"                       "Plot"                       "Trap"                      
#  [5] "Week"                       "mitogenome"                 "COI_Species"                "mock_ArcDyn"               
#  [9] "mapped_reads"               "mt_length"                  "sample"                     "well"                      
# [13] "samtools_filter"            "pathname"                   "Plate"                      "Sample_alias"              
# [17] "EI_sample_name"             "Concentration_ng_ul"        "Sample_volume_ul"           "qty_loaded_uL"             
# [21] "Quantification_method"      "x.260_280_ratio"            "lysis_buffer_orig_total_ul" "lysis_buffer_purified_ul"  
# [25] "lysis_buffer_proportion"    "sum_coverage"               "mean_coverage"              "stddev"                    
# [29] "coefvar"                    "length"                     "pathname_genomecov" 

# write_tsv(idx_meta_genomecov, "idx_meta_genomecov_PlatesGH_20180209.txt") # so if i need to read in a file during debuggging, here it is
...
# idx_meta_genomecov <- read_delim("idx_meta_genomecov_PlatesGH_20180209.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
...
names(idx_meta_genomecov)
#  [1] "Short_name_of_the_sample"                    "Year"                                       
#  [3] "Plot"                                        "Trap"                                       
#  [5] "Week"                                        "mitogenome"                                 
#  [7] "COI_Species"                                 "mock_ArcDyn"                                
#  [9] "mapped_reads"                                "mt_length"                                  
# [11] "sample"                                      "well"                                       
# [13] "samtools_filter"                             "pathname"                                   
# [15] "Plate"                                       "Sample_alias"                               
# [17] "EI_sample_name"                              "Concentration_ng_ul"                        
# [19] "Sample_volume_ul"                            "qty_loaded_uL"                              
...
# filter out samples with COI_spike_sum < 40 000 (summed over all 160 mitogenomes in the sample).  This removes 4 samples with very small numbers of COI_spike_sum and keeps the small ME50_1 mock community

sample_names <- sum_idx_meta_genomecov %>% dplyr::filter(COI_spike_sum < 40000) %>% dplyr::select(Short_name_of_the_sample)
samples_to_remove <- sample_names %>% dplyr::pull(Short_name_of_the_sample) # pull out a single variable
samples_to_remove # 4 samples to remove
idx_meta_genomecov <- idx_meta_genomecov %>% filter(!Short_name_of_the_sample %in% samples_to_remove) # 29830 obs

# sanity check:  186 samples, which is correct since i removed 4 samples
idx_meta_genomecov %>% distinct(Short_name_of_the_sample) %>% count()  # number of distinct values of Full_name_of_the_sample
...
# should be 4, and its 6.77, sd = 1.69
...
# should be 2, and it's 2.05, sd = 0.28
...
```{r}
# calculate COI_correction factor:  bombycid_mordellid_sum/min(bombycid_mordellid_sum)
...
idx_meta_genomecov <- idx_meta_genomecov %>% dplyr::select(Year:mapped_reads, mapped_reads_COI_corr, mapped_reads_COI_lysis_corr, mean_coverage, stddev, coefvar, Short_name_of_the_sample, length, mt_length:COI_corr)  
...
# [21] "Plate"                                       "Sample_alias"                               
# [23] "EI_sample_name"                              "Concentration_ng_ul"                        
# [25] "Sample_volume_ul"                            "qty_loaded_uL"                              
...
# [37] "COI_spike_sum"                               "ratio_mordellid20_bombyx10"                 
# [39] "ratio_elaterid40_bombyx"                     "ratio_elaterid40_mordellid20"               
# [41] "elaterid_mordellid_sum"                      "COI_corr"  
```

Make wide table for visualisation
```{r tidyr make wide table}

# remove mock soup data
mocks_idx_meta_genomecov <- idx_meta_genomecov %>% dplyr::filter(mock_ArcDyn == "mock")
idx_meta_genomecov <- idx_meta_genomecov %>% dplyr::filter(mock_ArcDyn == "arcdyn")

# create variable from Year, Week, and Trap(ABC)
# stringr::str_pad(x, width = 2, side = "left", pad = "0")  # to ensure that the date always has two digits, e.g. 07.
idx_meta_genomecov$YearWeekTrap <- str_c(idx_meta_genomecov$Year, idx_meta_genomecov$Week, idx_meta_genomecov$Trap, sep = "_")
    
# create variable from Date and Trap(ABC) # commented out for future use
# # stringr::str_pad(x, width = 2, side = "left", pad = "0")  # to ensure that the date always has two digits, e.g. 07.
...
idxstats_by_Short_name_of_the_sample <- idx_meta_genomecov %>% dplyr::group_by(Short_name_of_the_sample) %>% dplyr::distinct(pathname) %>% dplyr::count() %>% dplyr::arrange(desc(n))  

# ideally, there should be a 1 in all rows of column "n"
idxstats_by_YearWeekTrap <- idx_meta_genomecov %>% dplyr::group_by(YearWeekTrap) %>% dplyr::distinct(pathname) %>% dplyr::count() %>% dplyr::arrange(desc(n))  
...
# Make wide table for visualisation (left to right should follow temporal order (YearWeekTrap))
idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide <- idx_meta_genomecov %>% dplyr::select(YearWeekTrap, mitogenome, mapped_reads_COI_lysis_corr) %>% dplyr::arrange(YearWeekTrap) %>% tidyr::spread(YearWeekTrap, mapped_reads_COI_lysis_corr) %>% dplyr::arrange(mitogenome)
...
# write_tsv(idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide, "idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide_PlatesGH_20180209.txt")
# 
# write_tsv(mocks_idx_meta_genomecov_mapped_reads_wide, "mocks_idx_meta_genomecov_mapped_reads_wide_PlatesGH_20180209.txt")
# 
# write_tsv(mocks_idx_meta_genomecov_mapped_reads_COI_corr_wide, "mocks_idx_meta_genomecov_mapped_reads_COI_corr_wide_PlatesGH_20180209.txt")
```
