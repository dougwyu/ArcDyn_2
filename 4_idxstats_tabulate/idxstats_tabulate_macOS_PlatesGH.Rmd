...
idx_mitogenome_table <- idx %>% group_by(well) %>% distinct(mitogenome) %>% count() %>% arrange(desc(n)) # should be 160
...
# commented-out code that i can use if i get Month and Date information from Tea
...
saveRDS(genomecoverages_summ, "genomecoverages_summ_PlatesGH_20180210.RDS", compress = TRUE)
# genomecoverages_summ <- readRDS("genomecoverages_summ_PlatesGH_20180210.RDS")
# 
...
# write_tsv(idx_meta_genomecov, "idx_meta_genomecov_PlatesGH_20180210.txt") # so if i need to read in a file during debuggging, here it is
# used for debugging, in case i screw up the dataset while writing code
# idx_meta_genomecov <- read_delim("idx_meta_genomecov_PlatesGH_20180210.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
```
...
# create variable from Date and Trap(ABC) # commented out for future use if i get the month and date information
# # stringr::str_pad(x, width = 2, side = "left", pad = "0")  # to ensure that the date always has two digits, e.g. 07.
...
# ideally, there should be a 1 in all rows of column "n"
idxstats_by_YearWeekTrap <- idx_meta_genomecov %>% dplyr::group_by(YearWeekTrap) %>% dplyr::distinct(pathname) %>% dplyr::count() %>% dplyr::arrange(desc(n));  View(idxstats_by_YearWeekTrap)
...
# write_tsv(idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide, "idx_meta_genomecov_mapped_reads_COI_lysis_corr_wide_PlatesGH_20180210.txt")
# 
# write_tsv(mocks_idx_meta_genomecov_mapped_reads_wide, "mocks_idx_meta_genomecov_mapped_reads_wide_PlatesGH_20180210.txt")
# 
# write_tsv(mocks_idx_meta_genomecov_mapped_reads_COI_corr_wide, "mocks_idx_meta_genomecov_mapped_reads_COI_corr_wide_PlatesGH_20180210.txt")
```
